...
export async function waitForSettled(client, hash, opts = {}) {
  ...
  for (;;) {
    ...
    if (FINAL.has(status)) {
      // A final STATUS can arrive before the final RESULT. Measured 2026-08-27:
      // a deploy read `ACCEPTED / vote AGREE / NOT_VOTED`, and the same
      // transaction re-fetched moments later read FINISHED_WITH_RETURN — the
      // caller had already declared it dead. Give the execution result a grace
      // period to catch up with the status; return what we have when it
      // doesn't, and let succeeded() judge that honestly.
      const exec = tx.txExecutionResultName ?? tx.txExecutionResult;
      // vote IDLE on a fresh ACCEPTED is the same illness as exec NOT_VOTED:
      // measured 2026-08-27, a feed() read `ACCEPTED / vote IDLE` at 155 s and
      // `FINALIZED / vote AGREE` re-fetched minutes later. Only ACCEPTED gets
      // the benefit of the doubt — FINALIZED / IDLE really is an abandoned
      // transaction (see succeeded()).
      const voteStillForming =
        exec === "NOT_VOTED" || (status === "5" && Number(tx.result) === 0);
      if (voteStillForming && Date.now() - started < timeoutMs) {
        // the exec name catches up in seconds; a forming VOTE was measured
        // taking ~6 minutes on a stalling network, so it gets a longer leash
        const graceEnds =
          Date.now() + (exec === "NOT_VOTED" ? 45_000 : 300_000);
        let reopened = false;
        while (Date.now() < graceEnds) {
          await new Promise((r) => setTimeout(r, intervalMs));
          try {
            const again = await client.getTransaction({ hash });
            const s2 = String(again?.status ?? "");
            const e2 = again.txExecutionResultName ?? again.txExecutionResult;
            if (!FINAL.has(s2)) {
              // consensus reopened it (an appeal) — back to ordinary polling
              reopened = true;
              break;
            }
            const stillForming =
              e2 === "NOT_VOTED" || (s2 === "5" && Number(again.result) === 0);
            if (!stillForming) return again;
          } catch {
            // keep waiting; the grace window bounds this loop
          }
        }
